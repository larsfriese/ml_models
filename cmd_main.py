...
def main():
    print(colored('[0]', 'green'),'Make Model;           -dataset_filepath -feature_to_predict -list_of_features -epochs -optimizer [adam] -loss func [binary_crossentropy] -dropout [t/f] -save_model [t/f]')
    print(colored('[1]', 'green'),'Predict model;        -model_filepath -dataset_filepath -feature_to_predict -list_of_features -row_to_predict -nth element')
    print(colored('[2]', 'green'),'Deep model analysis;  -model_filepath -dataset_filepath -feature_to_predict -list_of_features -nth element -nth row element')
    ...
    while True:
        ...
        if str(i[0])=='0':
            ...
            #print(f'Features: {int((len(results[1])-1)/2)}')
            ...
            while second_loop==True:
                ...
                if str(x[0])=='0':
                    ...
                    fig = px.scatter(df, x=results[1][int(x[1])+2], y=results[1][int(x[2])+2], color=results[2], color_continuous_scale=px.colors.diverging.Tealrose, hover_name=results[2])
                    fig.show()
                elif str(x[0])=='2':
            ...
            while second_loop==True:
                ...
                if str(x[0])=='0':
                    ...
                    fig = px.scatter(df, x=results[int(x[1])+2], y=results[int(x[2])+2], color=column_to_predict, color_continuous_scale=px.colors.diverging.Tealrose, hover_name=column_to_predict)
                    fig.show()
                elif str(x[0])=='2':
                    second_loop=False
                else:
